...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n) mult_counter <= 'd0;
		...
		if(kernel_counter == 'd2 && (layer_counter == 'd1 || layer_counter == 'd3)) begin
...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n) layer_counter <= 'd0;
		...
		if(kernel_counter == 'd2) begin
			if(out_save_counter == 'd31) layer_counter <= layer_counter + 'd1;
			else if(layer_counter == 'd1) layer_counter <= 'd0;
			else if(layer_counter == 'd3) layer_counter <= 'd2;
			else layer_counter <= layer_counter + 'd1;
		end 
	end
...
always @(posedge clk or negedge rst_n) begin
	...
	else if(out_save_counter == 'd47) out_flag <= 'd1;
...
always @(*) begin
    if (!rst_n) next_state = IDLE;
        ...
        case (current_state)
			...
			LOAD  : next_state = (laod_counter == 'd17 && in_valid_k)? CAL:LOAD;
