...
// wire [inst_sig_width+inst_exp_width:0] exp_x, exp_nx;
// wire [inst_sig_width+inst_exp_width:0] one_add_exp_nx, exp_x_add_exp_nx, exp_x_sub_exp_nx;
// wire [inst_sig_width+inst_exp_width:0] Leaky_ReLU, tanh, sigmoid;
...
DW_fp_add #(inst_sig_width, inst_exp_width, inst_ieee_compliance) ADD(.a(((!inOpt[0])? one: exp_x)), .b(exp_nx), .rnd(3'b000), .z(add_exp_nx));
DW_fp_sub #(inst_sig_width, inst_exp_width, inst_ieee_compliance) SUB(.a(exp_x), .b(exp_nx), .rnd(3'b000), .z(exp_x_sub_exp_nx));
DW_fp_div #(inst_sig_width, inst_exp_width, inst_ieee_compliance) DIV(.a(((!inOpt[0])? one: exp_x_sub_exp_nx)), .b(add_exp_nx), .z(divide), .rnd(3'b000));

DW_fp_mult #(inst_sig_width, inst_exp_width, inst_ieee_compliance) ReLU (.a(acti_in), .b(z_one), .rnd(3'b000), .z(ReLU_out));
// DW_fp_exp #(inst_sig_width, inst_exp_width, inst_ieee_compliance) EXPPX(.a(acti_in), .z(exp_x));
// DW_fp_exp #(inst_sig_width, inst_exp_width, inst_ieee_compliance) EXPNX(.a({~acti_in[31], acti_in[30:0]}), .z(exp_nx));
// DW_fp_add #(inst_sig_width, inst_exp_width, inst_ieee_compliance) ADD_S(.a(one), .b(exp_nx), .rnd(3'b000), .z(one_add_exp_nx));
// DW_fp_add #(inst_sig_width, inst_exp_width, inst_ieee_compliance) ADD_T(.a(exp_x), .b(exp_nx), .rnd(3'b000), .z(exp_x_add_exp_nx));
// DW_fp_sub #(inst_sig_width, inst_exp_width, inst_ieee_compliance) SUB_T(.a(exp_x), .b(exp_nx), .rnd(3'b000), .z(exp_x_sub_exp_nx));
// DW_fp_div #(inst_sig_width, inst_exp_width, inst_ieee_compliance) DIV_T(.a(exp_x_sub_exp_nx), .b(exp_x_add_exp_nx), .z(tanh), .rnd(3'b000));
// DW_fp_div #(inst_sig_width, inst_exp_width, inst_ieee_compliance) DIV_S(.a(one), .b(one_add_exp_nx), .z(sigmoid), .rnd(3'b000));

// DW_fp_mult #(inst_sig_width, inst_exp_width, inst_ieee_compliance) M9 (.a(acti_in), .b(z_one), .rnd(3'b000), .z(Leaky_ReLU));
//---------------------------------------------------------------------
//   DESIGN
//---------------------------------------------------------------------
...
// 	case (inOpt)
// 		'b00: out_temp = (acti_in[31])? zero: acti_in;
// 		'b01: out_temp = (acti_in[31])? Leaky_ReLU: acti_in;
// 		'b10: out_temp = sigmoid;
// 		'b11: out_temp = tanh;
// 		default: out_temp = zero;
// 	endcase
// end
...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n)begin
		...
		for(i=14; i>=0; i=i-1)begin
			inImage1[i] <= inImage1[i+1];
			inImage2[i] <= inImage2[i+1];
			inImage3[i] <= inImage3[i+1];
		end
		// case (laod_counter)
		// 	0: begin
		// 		inImage1[0] <= Image1; 
