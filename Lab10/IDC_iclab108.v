...
//   GATE CLK
//---------------------------------------------------------------------
wire clk_mid, clk_avg, clk_rot, clk_flip, clk_shift, clk_region, clk_out, clk_op;
wire sleep_mid, sleep_avg, sleep_rot, sleep_flip, sleep_shift, sleep_region, sleep_out, sleep_op;
wire sleep_register, clk_register;

GATED_OR GATED_mid     ( .CLOCK(clk), .SLEEP_CTRL(cg_en&&sleep_mid     ), .RST_N(rst_n), .CLOCK_GATED(clk_mid ));
GATED_OR GATED_avg     ( .CLOCK(clk), .SLEEP_CTRL(cg_en&&sleep_avg     ), .RST_N(rst_n), .CLOCK_GATED(clk_avg ));
GATED_OR GATED_rot     ( .CLOCK(clk), .SLEEP_CTRL(cg_en&&sleep_rot     ), .RST_N(rst_n), .CLOCK_GATED(clk_rot ));
GATED_OR GATED_flip    ( .CLOCK(clk), .SLEEP_CTRL(cg_en&&sleep_flip    ), .RST_N(rst_n), .CLOCK_GATED(clk_flip));
GATED_OR GATED_shift   ( .CLOCK(clk), .SLEEP_CTRL(cg_en&&sleep_shift   ), .RST_N(rst_n), .CLOCK_GATED(clk_shift));
GATED_OR GATED_region  ( .CLOCK(clk), .SLEEP_CTRL(cg_en&&sleep_region  ), .RST_N(rst_n), .CLOCK_GATED(clk_region));
GATED_OR GATED_out     ( .CLOCK(clk), .SLEEP_CTRL(cg_en&&sleep_out     ), .RST_N(rst_n), .CLOCK_GATED(clk_out));
GATED_OR GATED_op      ( .CLOCK(clk), .SLEEP_CTRL(cg_en&&sleep_op      ), .RST_N(rst_n), .CLOCK_GATED(clk_op));
GATED_OR GATED_register( .CLOCK(clk), .SLEEP_CTRL(cg_en&&sleep_register), .RST_N(rst_n), .CLOCK_GATED(clk_register));

assign sleep_mid      = !(current_state == MID) ;
assign sleep_avg      = !(current_state == AVG) ;
assign sleep_rot      = !(current_state == ROT) ;
assign sleep_flip     = !(current_state == FLIP) ;
assign sleep_shift    = !(current_state == SHIFT || current_state == IDLE) ;
assign sleep_region   = !(current_state == GET_ANS || current_state == GET_IMG);
assign sleep_out      = !(current_state == OUT) ;
assign sleep_op       = !(current_state == SHIFT || current_state == GET_ANS || next_state == LOAD || current_state == IDLE) ;
assign sleep_register = !(current_state == WB || next_state == LOAD || current_state == IDLE);

// wire sleep_st;
// assign sleep_st = (next_state == LOAD || current_state == IDLE);
// assign sleep_stwb = (current_state == WB);
// generate
// 	for(i = 0; i < 8; i = i + 1)begin
// 		for(j = 0; j < 8; j = j + 1)begin
// 			GATED_OR GATED_register( .CLOCK(clk), .SLEEP_CTRL(cg_en && sleep_register[i][j]), .RST_N(rst_n), .CLOCK_GATED(clk_register[i][j]));
// 			assign sleep_register[i][j] = !((sleep_stwb && reg_en[i][j]) || sleep_st);
// 		end 
// 	end
// endgenerate
...
Midpoint Midpoint_m( .clk(clk_mid), .rst_n(rst_n), .in_0(img_region[0]), .in_1(img_region[1]), .in_2(img_region[2]), .in_3(img_region[3]), 
					 .out_0(img_mid));

Average Average_m( .clk(clk_avg), .rst_n(rst_n), .in_0(img_region[0]), .in_1(img_region[1]), .in_2(img_region[2]), .in_3(img_region[3]), 
					 .out_0(img_avg));

Rotate Rotate_m( .clk(clk_rot), .rst_n(rst_n), .in_0(img_region[0]), .in_1(img_region[1]), .in_2(img_region[2]), .in_3(img_region[3]), .mode(operation_reg[0][0]), 
					 .out_0(img_rot[0]), .out_1(img_rot[1]), .out_2(img_rot[2]), .out_3(img_rot[3]));

Flip Flip_m( .clk(clk_flip), .rst_n(rst_n), .in_0(img_region[0]), .in_1(img_region[1]), .in_2(img_region[2]), .in_3(img_region[3]), 
...
for(i = 0; i < 8; i = i + 1)begin
	for(j = 0; j < 8; j = j + 1)begin
		if(i == 7 && j == 7)begin
			always @(posedge clk_register or negedge rst_n) begin
...
for(i = 0; i < 15; i = i + 1)begin
	if(i==14)begin
		always @(posedge clk_op or negedge rst_n) begin
...
// image region
always @(posedge clk_region or negedge rst_n) begin
	if(!rst_n)begin
		img_region[0] <= 0;
		img_region[1] <= 0;
		img_region[2] <= 0;
